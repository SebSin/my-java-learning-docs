...
## 18. Can the constructor Constructor be overridden?

The constructor Constructor cannot be inherited, so Override cannot be overridden, but Overload can be overridden.
...
Computer software systems are the mapping of real-life business on the computer, and real-life business is actually a process of object collaboration. Object-oriented programming is to organize and write program code according to objects in the same way as real business, so that the computer system can recognize and understand the program code organized and written in object mode, so that the business objects in real life can be mapped into the computer system.

Object-oriented programming languages ​​have four main characteristics: encapsulation, inheritance, abstraction, and polymorphism.

1) Package:

Encapsulation is the basis for ensuring that software components have excellent modularity. The goal of encapsulation is to achieve "high cohesion and low coupling" of software components and prevent the impact of changes caused by program interdependence. In object-oriented programming languages, objects are the most basic unit of encapsulation. Object-oriented encapsulation is clearer and more powerful than encapsulation in traditional languages. Object-oriented encapsulation is to encapsulate the code describing the properties and behavior of an object in a "module", that is, a class. The properties are defined with variables, and the behavior is defined with methods. Methods can directly access the properties in the same object. . Under normal circumstances, just remember to put the variable and the method to access the variable together, and define all member variables in a class as private. Only the class's own methods can access these member variables. This is basically By implementing the encapsulation of objects, it is easy to find out the methods to be assigned to this class, which is basically object-oriented programming. Grasp one principle: put methods and related methods that operate on the same thing in the same class, and put methods and the data they operate on in the same class.

For example, if a person wants to draw a circle on the blackboard, this involves a total of three objects: the person, the blackboard, and the circle. Which object should the method of drawing a circle be assigned to? Since drawing a circle requires the use of the center and radius, the center and radius are obviously attributes of the circle. If they are defined as private member variables in the class, then the method of drawing a circle must be assigned to the circle before it can access the center and radius. For these two attributes of radius, people will only call the circle drawing method of the circle to send messages to the circle. The circle drawing method should not be allocated to the object of the person. This is object-oriented encapsulation, that is, the object is encapsulated into a A highly autonomous and relatively closed individual, the object state (properties) is read and changed by the object's own behavior (methods). A more easy-to-understand example is that the driver brakes the train. Should the braking action be assigned to the driver or to the train? Obviously, it should be assigned to the train, because the driver himself cannot have that much strength to brake a train. To stop, only the train itself can complete this action. The train needs to use multiple internal components such as clutches and brake pads to cooperate to complete the braking action. The driver's braking process only sends a message to the train, notifying the train to perform braking. Just an action.

2) Inheritance:

When defining and implementing a class, you can do it on the basis of an existing class. You can use the content defined by the existing class as your own content, and you can add some new content or modify the original method. Making it more suitable for special needs is called inheritance. Inheritance is a mechanism for subclasses to automatically share parent class data and methods. This is a relationship between classes that improves the reusability and scalability of software.

3) Abstraction:

Abstraction is to find out the similarities and commonalities of some things, and then classify these things into a class. This class only considers the similarities and commonalities of these things, and ignores those aspects that are irrelevant to the current topic and goal. Focus on aspects related to current goals. For example, if you see an ant and an elephant and you can imagine how they are similar, that is abstraction. Abstraction includes two aspects: behavioral abstraction and state abstraction. For example, define a Person class as follows:
```java
class Person{
...
People are inherently complex things with many aspects, but because the current system only needs to know the person's name and age, the class defined above only contains the two attributes of name and age. This is a kind of abstraction. Using abstraction You can avoid thinking about details that are irrelevant to your goals. My understanding of abstraction is not to use a microscope to look at all aspects of a thing, which would involve too much content, but to be good at dividing the boundaries of the problem, and only consider what the current system needs.

4) Polymorphism:

Polymorphism means that the specific type pointed to by the reference variable defined in the program and the method calls issued through the reference variable are not determined during programming, but are determined during the running of the program, that is, which class a reference variable will point to. Instance object, the method call issued by the reference variable is a method implemented in which class, it must be determined during the running of the program. Because the specific class is determined only when the program is running, the reference variable can be bound to various class implementations without modifying the source program code, causing the specific method called by the reference to change accordingly, that is, it does not need to be modified. The program code can change the specific code bound to the program when it is running, allowing the program to select multiple running states. This is polymorphism. Polymorphism enhances software flexibility and scalability. For example, UserDao in the following code is an interface. It defines that the instance object pointed to by the reference variable userDao is returned by daofactory.getDao() during execution. Sometimes it points to the implementation of UserJdbcDao, and sometimes it points to the implementation of UserHibernateDao. In this way, without modifying the source code, you can change the specific class implementation pointed by userDao, which will cause the specific code called by userDao.insertUser() method to also change, that is, sometimes the insertUser method of UserJdbcDao is called, and sometimes it is called UserHibernateDao's insertUser method:

UserDao userDao = daofactory.getDao();

userDao.insertUser(user);

Metaphor: When a person eats, do you see the left hand or the right hand?
...
6) Both abstract classes and interfaces can contain static member variables. The access types of static member variables in abstract classes can be arbitrary, but variables defined in interfaces can only be of public static final type, and the default is public static final type.
...
public abstract class BaseServlet extends HttpServlet {
    ...
    public final void service(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        //Record access logs and perform permission judgment
        if (has permission) {
            try {
                doService(request, response);
            } catch (Exception e) {
                // todo: record exception information
            }
        }
    }
    ...
    protected abstract void doService(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        //Note that the access permission is defined as protected, which looks professional and rigorous because it is specially used for subclasses.
    }
}
```
...
public class MyServlet1 extends BaseServlet {
    protected void doService(HttpServletRequest request, HttpServletResponse response) throws IOExcetion, ServletException {
        // This Servlet only handles specific business logic code
    }
}
```
A certain section of code in the middle of the parent class method is uncertain and is left to the subclass, so the template method is used to design the pattern.
Note: The idea of ​​this question is to first explain the basic concepts of abstract classes and interfaces in general, then compare the grammatical details of the two, and finally talk about the application differences between the two. The method for comparing the differences in grammatical details between the two is: first compare and answer one by one from six aspects such as constructors, ordinary member variables and methods (including abstract methods), static variables and methods, and inheritance in a class, and then start from the third Answer from the perspective of inheritance, especially at the end using a typical example to show my profound technical skills.
